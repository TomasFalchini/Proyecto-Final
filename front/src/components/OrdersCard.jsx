    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        if (b.length > 0) {
          ...
          Swal.fire({
            icon: "info",
            text: `We do not have the required quantities of the following plants. We load the stock in existence to the cart: ${b.map(
              (item) => {
      ...
      {props.data.length === 0 ? null : (
        <div>
          <div className={s.date}>
            <h3 className={s.state}>
              {" "}
              <strong>Order State:</strong> {props.state}
            </h3>
            {props.state === "Pending" ? (
              <button onClick={() => navigate(`/cart`)} className={s.edit}>
                Edit
              </button>
            ) : null}
            {props.state !== "Pending" ? (
                ...
                <span>
                  {new Date(
                    props.date?._seconds * 1000 +
                      props.date._nanoseconds / 1000000
                  ).toLocaleDateString()}
                </span>
                <span>
                  {new Date(
